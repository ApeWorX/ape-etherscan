...
ETHERSCAN_API_URI = (
    lambda n: (f"https://api-{n}.etherscan.io/api" if n != "mainnet" else "https://api.etherscan.io/api")
)
...
class Etherscan(ExplorerAPI):
    ...
    def get_contract_abi(self, address: str) -> List[ABI]:
        response = requests.get(ETHERSCAN_API_URI(self.network.name),params={"module":"contract","action":"getabi","address":address})
